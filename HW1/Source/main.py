...
def histogram_equalization(img_in):
    ...
    img_out = HistogramEqualization.HistogramEqualizationOfImage(img_in)
    return True, img_out
...
def low_pass_filter(img_in):
    ...
    if (len(img_in.shape) > 1):
        ...
        img_in_gray = img_in
    img_out = ImageFiltering.LowPassFilter(img_in_gray)
    return True, img_out
...
def high_pass_filter(img_in):
    ...
    if (len(img_in.shape) > 1):
        ...
        img_in_gray = img_in
    img_out = ImageFiltering.HighPassFilter(img_in_gray)
    return True, img_out
...
def deconvolution(img_in):
    ...
    kernel = ImageFiltering.GetKernel()
    img_out = ImageFiltering.Deconvolution(img_in, kernel)
    return True, img_out
...
def laplacian_pyramid_blending(img_in1, img_in2):
    ...
    dim = min(m1, n1, m2, n2)
    #img_in1 = img_in1[0:dim,0:dim,:]
    img_in1 = img_in1[:, :m1, :]
    #img_in2 = img_in2[0:dim,0:dim,:]
    img_in2 = img_in2[:m1, :m1, :]
    img_out = PyramidBlending.BlendImages(img_in1, img_in2)
    return True, img_out
