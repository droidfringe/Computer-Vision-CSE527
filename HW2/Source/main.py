...
import PyramidBlendingWithMask
...
def Perspective_warping(im1, im2, im3):
    ...
    (M, pts1, pts2, mask) = getTransform(im2, im1, method='homography')
    out = cv2.warpPerspective(
        im2,
        M, (im1.shape[1], im1.shape[0]),
        dst=im1.copy(),
        borderMode=cv2.BORDER_TRANSPARENT)
    #out = cv2.warpPerspective(im1, M, (im2.shape[1],im2.shape[0]))
    (M, pts1, pts3, mask) = getTransform(im3, im1, method='homography')
    ...
    #r = RMSD_Rectified.RMSD(1, out2, master)
    #print(r)
    return True
...
def Bonus_perspective_warping(im1, im2, im3):
    ...
    # Write your codes here
    #output_image = Perspective_warping(img1, img2, img3) # This is dummy output, change it to your output
    im1Mask = np.ones(im1.shape, dtype=im1.dtype)
    im2Mask = np.ones(im2.shape, dtype=im2.dtype)
    im3Mask = np.ones(im3.shape, dtype=im3.dtype)
    im1 = cv2.copyMakeBorder(im1, 200, 200, 500, 500, cv2.BORDER_CONSTANT)
    im1Mask = cv2.copyMakeBorder(im1Mask, 200, 200, 500, 500,
                                 cv2.BORDER_CONSTANT)
    (M, pts1, pts2, mask) = getTransform(im2, im1, method='homography')
    out = cv2.warpPerspective(
        im2,
        ...
        dst=im1.copy(),
        borderMode=cv2.BORDER_TRANSPARENT)
    im2Proj = cv2.warpPerspective(
        im2, M, (im1.shape[1],
                 im1.shape[0]))  #, borderMode=cv2.BORDER_TRANSPARENT)
    im2MaskProj = cv2.warpPerspective(
        im2Mask, M, (im1.shape[1],
                     im1.shape[0]))  #, borderMode=cv2.BORDER_TRANSPARENT)
    outBlended = PyramidBlendingWithMask.Laplacian_Pyramid_Blending_with_mask(
        im2Proj, im1, im2MaskProj.astype(np.float32), 4)
    #outBlended = PyramidBlending.BlendImagesWithMask(im2Proj, im1, im2MaskProj.astype(np.float32), 4)
    '''
    plt.figure(1)
    plt.subplot(121)
    plt.imshow(out, cmap='gray')
    plt.subplot(122)
    plt.imshow(outBlended, cmap='gray')
    plt.show()
    '''
    #out = cv2.warpPerspective(im1, M, (im2.shape[1],im2.shape[0]))
    (M, pts1, pts3, mask) = getTransform(im3, im1, method='homography')
    #out2 = cv2.warpPerspective(im3, M, (im1.shape[1],im1.shape[0]), dst=out.copy(), borderMode=cv2.BORDER_TRANSPARENT)
    im3Proj = cv2.warpPerspective(
        im3, M, (im1.shape[1],
                 im1.shape[0]))  #, borderMode=cv2.BORDER_TRANSPARENT)
    im3MaskProj = cv2.warpPerspective(
        im3Mask, M, (im1.shape[1],
                     im1.shape[0]))  #, borderMode=cv2.BORDER_TRANSPARENT)
    outBlended2 = PyramidBlendingWithMask.Laplacian_Pyramid_Blending_with_mask(
        im3Proj, outBlended, im3MaskProj.astype(np.float32), 4)
    # Write out the result
    output_name = sys.argv[5] + "output_homography_lpb.png"
    cv2.imwrite(output_name, outBlended2)
...
def Cylindrical_warping(im1, im2, im3):
    ...
    cyl_mask1 = cyl_mask1 > 1
    cyl_mask1 = cyl_mask1.astype(np.uint8)
    cyl2, cyl_mask2 = cylindricalWarpImage(im2, K)
    cyl_mask2 = cyl_mask2 > 1
    cyl_mask2 = cyl_mask2.astype(np.uint8)
    cyl3, cyl_mask3 = cylindricalWarpImage(im3, K)
    cyl_mask3 = cyl_mask3 > 1
    cyl_mask3 = cyl_mask3.astype(np.uint8)
    cyl1 = cv2.copyMakeBorder(cyl1, 50, 50, 300, 300, cv2.BORDER_CONSTANT)
    cyl_mask1 = cv2.copyMakeBorder(cyl_mask1, 50, 50, 300, 300,
                                   cv2.BORDER_CONSTANT)
    (M, pts1, pts2, mask) = getTransform(cyl2, cyl1, method='affine')
    out = cv2.warpAffine(
        cyl2, M[0:2, :],
        (cyl1.shape[1],
         cyl1.shape[0]))  #, dst=cyl1.copy(), borderMode=cv2.BORDER_TRANSPARENT)
    out_mask = cv2.warpAffine(cyl_mask2, M[0:2, :], (cyl1.shape[1],
                                                     cyl1.shape[0]))
    cyl1_mask_fin = np.subtract(cyl_mask1, np.multiply(out_mask, cyl_mask1))
    out_fin = np.multiply(cyl1_mask_fin, cyl1) + out
    out_fin_mask = (np.add(out_mask, cyl_mask1) > 0).astype(np.uint8)

    (M, pts1, pts3, mask) = getTransform(cyl3, cyl1, method='affine')
    out = cv2.warpAffine(cyl3, M[0:2, :], (cyl1.shape[1], cyl1.shape[0]))
    out_mask = cv2.warpAffine(cyl_mask3, M[0:2, :], (cyl1.shape[1],
                                                     cyl1.shape[0]))
    cyl1_mask_fin = np.subtract(out_fin_mask,
                                np.multiply(out_mask, out_fin_mask))
    out_fin2 = np.multiply(cyl1_mask_fin, out_fin) + out

    # Write out the result
    output_name = sys.argv[5] + "output_cylindrical.png"
    cv2.imwrite(output_name, out_fin2)
    #master = cv2.imread('example_output2.png',0)
    #r = RMSD_Rectified.RMSD(2, out_fin2, master)
    #print r
    return True
...
def Bonus_cylindrical_warping(im1, im2, im3):
    ...
    cyl_mask1 = cv2.copyMakeBorder(cyl_mask1, 50, 50, 300, 300,
                                   cv2.BORDER_CONSTANT)
    (M, pts1, pts2, mask) = getTransform(cyl2, cyl1, method='affine')
    im2Proj = cv2.warpAffine(
        cyl2, M[0:2, :],
        (cyl1.shape[1],
         cyl1.shape[0]))  #, dst=cyl1.copy(), borderMode=cv2.BORDER_TRANSPARENT)
    im2MaskProj = cv2.warpAffine(cyl_mask2, M[0:2, :], (cyl1.shape[1],
                                                        cyl1.shape[0]))
    outBlended = PyramidBlendingWithMask.Laplacian_Pyramid_Blending_with_mask(
        im2Proj, cyl1, im2MaskProj.astype(np.float32), 3)
    #cyl1_mask_fin = np.subtract(cyl_mask1, np.multiply(out_mask, cyl_mask1))
    #out_fin = np.multiply(cyl1_mask_fin, cyl1) + out
    #out_fin_mask = (np.add(out_mask,cyl_mask1) > 0).astype(np.uint8)
    '''
    ...
    plt.subplot(132)
    plt.imshow(im2Proj, cmap='gray')
    plt.subplot(133)
    plt.imshow(outBlended, cmap='gray')
    plt.show()
    '''
    (M, pts1, pts3, mask) = getTransform(cyl3, cyl1, method='affine')
    im3Proj = cv2.warpAffine(cyl3, M[0:2, :], (cyl1.shape[1], cyl1.shape[0]))
    im3MaskProj = cv2.warpAffine(cyl_mask3, M[0:2, :], (cyl1.shape[1],
                                                        cyl1.shape[0]))
    #cyl1_mask_fin = np.subtract(out_fin_mask, np.multiply(out_mask, out_fin_mask))
    #out_fin2 = np.multiply(cyl1_mask_fin, out_fin) + out
    outBlended2 = PyramidBlendingWithMask.Laplacian_Pyramid_Blending_with_mask(
        im3Proj, outBlended, im3MaskProj.astype(np.float32), 3)
    # Write out the result
