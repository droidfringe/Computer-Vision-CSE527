...
def produce_binary_mask(img_marking, superpixels, color_hists, norm_hists):
    ...
    return segmask
global drawingFG, drawingBG, drawnFG, drawnBG, img_marking, img_showing
drawingFG = False
drawingBG = False
drawnFG = False
drawnBG = False
...
def capture_click(event,x,y,flags,param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if drawingFG:
            ...
            cv2.circle(img_marking,(x,y),3,(0,0,255),-1)
            cv2.circle(img_showing,(x,y),3,(0,0,255),-1)
            drawnFG = True
        elif drawingBG:
            #print 'Processing right'
            cv2.circle(img_marking,(x,y),3,(255,0,0),-1)
            cv2.circle(img_showing,(x,y),3,(255,0,0),-1)
            drawnBG = True
...
if __name__ == '__main__':
    ...
    img_marking = 255*np.ones_like(img)
    img_showing = img.copy()
    ...
    cv2.setMouseCallback('Interactive Segmentation', capture_click)
    print 'Use left click for specifying foreground'
    print 'Use right click for specifying background'
    print 'After one background and foreground is specified, mask will be shown'
    #print 'After specifying background and foreground, press s to perform segmentation'
    print 'Press r to reset'
    print 'Press esc to exit'
    while(1):
        #cv2.imshow('Interactive Segmentation', img_marking)
        cv2.imshow('Interactive Segmentation', img_showing)
        k = cv2.waitKey(1) & 0xFF
        if(drawnBG and drawnFG):
            mask = produce_binary_mask(img_marking, superpixels, color_hists, norm_hists)
